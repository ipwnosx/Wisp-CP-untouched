<?php
    ...
    if(isset($list) && $list){
        foreach($list AS $i=>$row){
            ...
            $detail_link = Controllers::$init->AdminCRLink("manage-website-2",["references","edit-category"])."?id=".$id;

            $catLink            = Controllers::$init->CRLink("references_category",[$row["route"]]);
            $catLink            = ' <a href="'.$catLink.'" target="_blank" class="targeturl sbtn"><i class="fa fa-external-link" aria-hidden="true"></i></a>';
            if($row["parent_route"] == '')
                $parent_catLink = NULL;
            else{
                $parent_catLink    = Controllers::$init->CRLink("references_category",[$row["parent_route"]]);
