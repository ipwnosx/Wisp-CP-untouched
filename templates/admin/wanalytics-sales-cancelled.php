            ...
            <div class="reports">
                ...
                <div class="reports-content">
                    ...
                    <div class="icerikbaslik">
                        ...
                        <div class="reportsdate">
                            <form action="<?php echo $links["controller"]; ?>" method="get" id="FilterForm">
                                ...
                                <input type="hidden" name="period" value="<?php echo $s_period; ?>">
                                <label for="DateRange">
                    ...
                    <div class="reportsinfotitle">
                        <?php echo __("admin/wanalytics/report-note-14"); ?>
                    </div>

                    <div class="reportfoundinfo">
                        <h4>
                            <?php
                                echo __("admin/wanalytics/report-note-13",[
                                    '{total}'       => $result_total,
                                    '{count}'       => $result_count,
                    ...
                    <div class="selprodreport">
                        <?php
                            ...
                            if(isset($s_period) && $s_period)
                                $filtered_link = str_replace("&period=".$s_period,"&period={2}",$filtered_link);
                            else
                                $filtered_link .= "&period={2}";
                        ?>
                        ...
                        <div class="formcon">
                            ...
                            <div class="yuzde70">
                                <select name="product" onchange="location = this.options[this.selectedIndex].value;">
                                    <option value="<?php echo Utility::text_replace($filtered_link,['{1}' => '','{2}' => $s_period]); ?>"><?php echo ___("needs/allOf"); ?></option>
                                    <?php
                                        if(Config::get("options/pg-activation/hosting")){
                                            ...
                                            if($products){
                                                foreach ($products as $product){
                                                    ?>
                                                    <option<?php echo $s_product == 'hosting/'.$product["id"] ? ' selected' : ''; ?> value="<?php echo Utility::text_replace($filtered_link,['{1}' => 'hosting/'.$product["id"],'{2}' => $s_period]);?>"><?php echo $line."» ".$product["title"]; ?></option>
                                                    <?php
                                                }
                                            }
                                        ...
                                        if(Config::get("options/pg-activation/server")){
                                            ...
                                            if($products){
                                                foreach ($products as $product) {
                                                    ?>
                                                    <option<?php echo $s_product == 'server/'.$product["id"] ? ' selected' : ''; ?> value="<?php echo Utility::text_replace($filtered_link,['{1}' => 'server/'.$product["id"],'{2}' => $s_period]);?>"><?php echo $line."» ".$product["title"]; ?></option>
                                                    <?php
                                                }
                                            }
                                        ...
                                        if(Config::get("options/pg-activation/software")){
                                            ...
                                            if($products){
                                                foreach ($products as $product) {
                                                    ?>
                                                    <option<?php echo $s_product == 'software/'.$product["id"] ? ' selected' : ''; ?> value="<?php echo Utility::text_replace($filtered_link,['{1}' => 'software/'.$product["id"],'{2}' => $s_period]);?>"><?php echo $line."» ".$product["title"]; ?></option>
                                                    <?php
                                                }
                                            }
                                        ...
                                        if(Config::get("general/local")=="tr" && Config::get("options/pg-activation/sms")){
                                            ...
                                            if($products){
                                                foreach ($products as $product) {
                                                    ?>
                                                    <option<?php echo $s_product == 'sms/'.$product["id"] ? ' selected' : ''; ?> value="<?php echo Utility::text_replace($filtered_link,['{1}' => 'sms/'.$product["id"],'{2}' => $s_period]);?>"><?php echo $line."» ".$product["title"]; ?></option>
                                                    <?php
                                                }
                                            }
                                        }
                                        ...
                                        if($pspecialGroups){
                                            foreach($pspecialGroups AS $category){
                                                ...
                                                if($products){
                                                    foreach ($products as $product) {
                                                        ?>
                                                        <option<?php echo $s_product == 'special/'.$product["id"] ? ' selected' : ''; ?> value="<?php echo Utility::text_replace($filtered_link,['{1}' => 'special/'.$product["id"],'{2}' => $s_period]);?>"><?php echo $line."» ".$product["title"]; ?></option>
                                                        <?php
                                                    }
                                                }
                                        ...
                                        if(Config::get("options/pg-activation/domain")){
                                            ...
                                            if($tlds){
                                                foreach($tlds AS $tld){
                                                    ?>
                                                    <option<?php echo $s_product == 'domain/'.$tld["id"] ? ' selected' : ''; ?> value="<?php echo Utility::text_replace($filtered_link,['{1}' => 'domain/'.$tld["id"],'{2}' => $s_period]);?>">» <?php echo $tld["name"]; ?></option>
                                                    <?php
                                                }
                                            }
                                        }
                        ...
                        <div class="formcon">
                            <div class="yuzde30"><?php echo __("admin/wanalytics/element-period"); ?></div>
                            <div class="yuzde70">
                                <select onchange="location = this.options[this.selectedIndex].value;">
                                    <option value="<?php echo Utility::text_replace($filtered_link,['{1}' => $s_product,'{2}' => '']); ?>"><?php echo ___("needs/allOf"); ?></option>
                                    <?php
                                        $periods = [
                                            'monthly',
                                            '3_monthly',
                                        ...
                                        foreach($periods AS $row){
                                            $parse  = explode("_",$row);
                                            $num    = isset($parse[1]) ? $parse[0] : 0;
                                            $name   = isset($parse[1]) ? $parse[1] : $parse[0];

                                            if($name == 'monthly') $name = ___("date/period-month");
                                            if($name == 'yearly') $name = ___("date/period-year");
                                            if($name == 'onetime') $name = ___("date/period-none");

                                            if($num) $name = $num." ".$name;
                                            $link       = Utility::text_replace($filtered_link,['{1}' => $s_product,'{2}' => $row]);
                                            $selected   = '';
                                            if($row == $s_period)
                                                $selected = ' selected';
                                            ?>
                                            <option<?php echo $selected; ?> value="<?php echo $link; ?>"><?php echo $name;?></option>
                                            <?php
                                        }
                                    ?>
                                </select>
                    ...
                    <table width="100%" id="list_table" class="table table-striped table-borderedx table-condensed nowrap">
                        ...
                        <tr>
                            ...
                            <th align="center" data-orderable="false"><?php echo __("admin/wanalytics/list-table-total-sales"); ?></th>
                        </tr>
                        </thead>
                            ...
                            if(isset($list) && $list){
                                foreach($list AS $k=>$row){
                                    ...
                                    <tr>
                                        ...
                                        <td align="center">
                                            <strong><?php echo $row["total"]; ?></strong> (<?php echo $row["count"]; ?>)
                                        </td>
                                    </tr>
                                    <?php
                                }
