...
<script type="text/javascript">
    function OnlineNICSSL_open_tab(elem, tabName){
        var owner = "OnlineNICSSL_tab";
...
<div id="OnlineNICSSL_tab">
    <ul class="modules-tabs">
        <li><a href="javascript:OnlineNICSSL_open_tab(this,'detail');" data-tab="detail" class="modules-tab-item active"><?php echo $LANG["tab-detail"]; ?></a></li>
        <li><a href="javascript:OnlineNICSSL_open_tab(this,'import');" data-tab="import" class="modules-tab-item"><?php echo $LANG["tab-import"]; ?></a></li>
    </ul>

    <div id="OnlineNICSSL_tab-detail" class="modules-tabs-content" style="display: block">
        <form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="OnlineNICSSLSettings">
            <input type="hidden" name="operation" value="module_controller">
            <input type="hidden" name="module" value="OnlineNICSSL">
            ...
            <div class="formcon" style="display: none;">
                ...
                <div class="yuzde70">
                    <input<?php echo $CONFIG["settings"]["test-mode"] ? ' checked' : ''; ?> type="checkbox" name="test-mode" value="1" id="OnlineNICSSL_test-mode" class="checkbox-custom">
                    <label class="checkbox-custom-label" for="OnlineNICSSL_test-mode">
            ...
            <div style="float:left;" class="guncellebtn yuzde30"><a id="OnlineNICSSL_testConnect" href="javascript:void(0);" class="lbtn"><i class="fa fa-plug" aria-hidden="true"></i> <?php echo $LANG["test-button"]; ?></a></div>


            <div style="float:right;" class="guncellebtn yuzde30"><a id="OnlineNICSSL_submit" href="javascript:void(0);" class="yesilbtn gonderbtn"><?php echo $LANG["save-button"]; ?></a></div>

        </form>
        <script type="text/javascript">
            $(document).ready(function(){
                $("#OnlineNICSSL_testConnect").click(function(){
                    $("#OnlineNICSSLSettings input[name=controller]").val("test_connection");
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"OnlineNICSSL_handler",
                    });
                });

                $("#OnlineNICSSL_submit").click(function(){
                    $("#OnlineNICSSLSettings input[name=controller]").val("settings");
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"OnlineNICSSL_handler",
            ...
            function OnlineNICSSL_handler(result){
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                            if(solve.for != undefined && solve.for != ''){
                                $("#OnlineNICSSLSettings "+solve.for).focus();
                                $("#OnlineNICSSLSettings "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                                $("#OnlineNICSSLSettings "+solve.for).change(function(){
    ...
    <div id="OnlineNICSSL_tab-import" class="modules-tabs-content" style="display: none;">
        ...
        <script type="text/javascript">
            ...
            function OnlineNICSSL_import_handler(result){
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                            if(solve.for != undefined && solve.for != ''){
                                $("#OnlineNICSSLImport "+solve.for).focus();
                                $("#OnlineNICSSLImport "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                                $("#OnlineNICSSLImport "+solve.for).change(function(){
                                    $(this).removeAttr("style");
                                });
                            }
                            if(solve.message != undefined && solve.message != '')
            ...
            $(document).ready(function(){
                ...
                $("#OnlineNICSSL_import_submit").click(function(){
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"OnlineNICSSL_import_handler",
                    });
                });

                $('#OnlineNICSSL_list_ssl').DataTable({
                    "columnDefs": [
                        {
                            "targets": [0],
                            "visible":false,
        ...
        <form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="OnlineNICSSLImport">
            <input type="hidden" name="operation" value="module_controller">
            <input type="hidden" name="module" value="OnlineNICSSL">
            <input type="hidden" name="controller" value="import">

            <table width="100%" id="OnlineNICSSL_list_ssl" class="table table-striped table-borderedx table-condensed nowrap">
            ...
            <div class="guncellebtn yuzde20" style="float: right;">
                <a href="javascript:void(0);" id="OnlineNICSSL_import_submit" class="gonderbtn mavibtn"><?php echo $LANG["import-button"]; ?></a>
            </div>
